...
def descriptors(cif, calculation_type='bulk'):
    ...
    if type(cif) is str:
        ...
        structure = Structure(
            lattice=cif.cell, species=cif.symbols, coords=cif.positions)
    ...
    d1 = [volume_per_atom] if calculation_type == 'bulk' else []
    d2 = [empty_volume]if calculation_type == 'bulk' else []
    d3 = distance_matrix if calculation_type != 'formula' else []
    d4 = spacegroup_numbers_list if calculation_type == 'bulk' else []
